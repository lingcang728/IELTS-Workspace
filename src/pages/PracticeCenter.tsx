...
import { BookCatalog, SessionRow } from "../components/ExamRows";
...
import { completion, formatDay, isFinished, startOfWeek } from "../lib/format";
import type { ExamSummary, SessionSummary } from "../lib/types";
import type { CatalogModule, View } from "../lib/view";

export function PracticeCenter({ exams, sessions, busy, onStart, onRetake, onContinue, onView }: { exams: ExamSummary[]; sessions: SessionSummary[]; busy: boolean; onStart: (e: ExamSummary, mode: "mock" | "practice") => void; onRetake: (e: ExamSummary, mode: "mock" | "practice") => void; onContinue: (id: string) => void; onView: (v: View) => void }) {
  ...
  const unfinished = practiceSessions.find((s) => s.status !== "submitted");
  ...
  return <div className="practice-page page-stack"><PageHeading title="练习中心" subtitle="自由选择模块与题目，按自己的节奏暂停、重听和复盘" />
    <div className="practice-layout"><section className="workspace-card catalog-card"><CatalogToolbar module={module} setModule={setModule} query={query} setQuery={setQuery} /><div className="catalog-heading"><h2>可用练习</h2><span>{examCount} 套</span></div>{groups.length === 0 ? <div className="empty-state compact"><Icon name="search" /><p>没有找到符合条件的练习。</p></div> : <BookCatalog groups={groups} mode="practice" busy={busy} audioAction="添加音频" startAction="开始练习" onStart={(e) => onStart(e, "practice")} onRetake={(e) => onRetake(e, "practice")} />}</section>
