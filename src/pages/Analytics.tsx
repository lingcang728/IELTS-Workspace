    ...
    {!report || sessionCount === 0
      ...
      : <>
        <div className="analytics-grid">
          ...
          <section className="workspace-card module-score-card"><h2>各单项平均估算 Band</h2><div className="score-rings">{(["listening", "reading", "writing"] as const).map((m) => <div key={m} className={`score-ring ${m}`}><strong>{m === "writing" ? "—" : objective?.[m]?.toFixed(1) ?? "—"}</strong><span>{m === "listening" ? "听力" : m === "reading" ? "阅读" : "写作"}</span><small>{report.moduleCounts[m] ?? 0} 次</small></div>)}</div><small className="ring-note">写作不产生客观 Band，显示 —。口语未启用，不进入统计。</small></section>
