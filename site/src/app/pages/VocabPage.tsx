/**
 * 生词本 — web port of the desktop `Vocab` page. The review tab runs the
 * due-card flow (context cloze first, answer only after 显示答案, then a
 * 1–4 grade into `vocabReview`/FSRS); the list tab shows every card with a
 * manual add form (term + sentence + note → `vocabAdd`) and delete.
 */
import { useEffect, useMemo, useRef, useState } from "react";
import type { Route } from "../nav";
import { vocabAdd, vocabDelete, vocabDue, vocabList, vocabReview } from "../api";
import { formatDate } from "../lib/format";
import { cloze } from "../lib/review";
import { Icon, PageHeading } from "../components/ReviewShared";
import type { VocabCard, VocabGrade } from "../types";
...
export default function VocabPage({ route }: { route: Route }) {
  void route;
  ...
  async function add() {
    ...
    setTerm(""); setSentence(""); setNote("");
    await reload();
  }

  if (all === null) {
    return <div className="page-stack vocab-page">
      <PageHeading title="生词本" />
      {saveError
        ? <p className="form-error">{saveError}</p>
        : <div className="workspace-card empty-state compact"><p className="meta">正在读取生词本…</p></div>}
    </div>;
  }
    ...
    {tab === "review" && (card
      ? <section className="workspace-card flashcard">
          ...
          {flipped
            ...
            : <button type="button" className="primary-button flashcard-flip" onClick={() => setFlipped(true)}>显示答案</button>}
    ...
    {tab === "list" && <>
      <section className="workspace-card vocab-add">
        ...
        <div className="vocab-add-row">
          ...
          <input value={note} onChange={(e) => setNote(e.target.value)} placeholder="备注 / 释义（可留空）" style={{ gridColumn: "1 / -1" }} />
        </div>
      </section>
      <div className="vocab-list">{all.map((row) => <article className="workspace-card vocab-row" key={row.id}>
        <div>
          ...
          {row.note && <p className="vocab-sentence">{row.note}</p>}
      ...
      {all.length === 0 && <div className="workspace-card empty-state"><Icon name="pen" size={42} />
        <h2>生词本是空的</h2><p>在考场里选中一个词加入生词本，或在上方手动补录。</p></div>}
    </>}
  </div>;
}
