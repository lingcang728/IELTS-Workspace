/**
 * 错题本 — web port of the desktop `Mistakes` page. Open/mastered sections,
 * weakness card by open-mistake count (each row links into the filtered
 * library), per-row redo input scored by `attemptMatches` and persisted via
 * `mistakeResolve`, plus delete. Nothing is fabricated: rows come straight
 * from the mistakes store.
 */
import { useEffect, useMemo, useRef, useState } from "react";
import type { Route } from "../nav";
import { mistakeDelete, mistakeList, mistakeResolve } from "../api";
import { formatDate, questionTypeLabel } from "../lib/format";
import { attemptMatches } from "../lib/mistakes";
import { answerText, practiceTypeHref } from "../lib/review";
import { Icon, PageHeading } from "../components/ReviewShared";
import type { Mistake, ModuleKind } from "../types";

type Filter = "open" | "mastered" | "all";

export default function MistakesPage({ route }: { route: Route }) {
  void route;
  ...
  // The weakness view is the point of the book: open mistakes grouped by
  // question type, each row leading to the library filtered on that type.
  ...
  if (rows === null) {
    return <div className="page-stack mistakes-page">
      <PageHeading title="错题本" />
      {saveError
        ? <p className="form-error">{saveError}</p>
        : <div className="workspace-card empty-state compact"><p className="meta">正在读取错题本…</p></div>}
    </div>;
  }
  ...
  return <div className="page-stack mistakes-page">
    ...
    {byType.length > 0 && <section className="workspace-card weakness-card">
      <div className="card-heading"><h2>薄弱题型</h2><span className="meta">按待攻克错题数排序 · 点击去题库练习</span></div>
      <div className="weakness-list">{byType.slice(0, 6).map(([type, count]) => <a
        key={type} className="weakness-row" href={practiceTypeHref(type)} title={`去题库练 ${questionTypeLabel(type)}`}>
    ...
    {visible.length === 0
      ...
      : <div className="mistake-list">{visible.map((row) => <article className="workspace-card mistake-row" key={row.id}>
          ...
          <div className="mistake-redo">
            ...
            <a className="link-button" href={practiceTypeHref(row.questionType)}><Icon name="arrow" size={14} />练这个题型</a>
