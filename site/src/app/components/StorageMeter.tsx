...
import { formatBytes } from "../lib/dataOps";
  ...
  return (
    <div className="storage-meter">
      <div className="storage-meter-head">
        <strong>{usage != null ? formatBytes(usage) : "—"}</strong>
        <span>
          {quota != null
            ? `配额约 ${formatBytes(quota)}${ratio != null ? ` · 已用 ${Math.round(ratio * 100)}%` : ""}`
            : "浏览器本地存储"}
        </span>
      </div>
      {ratio != null && (
        ...
        >
          <i style={{ width: `${Math.max(1, Math.round(ratio * 100))}%` }} />
        </div>
      )}
      <ul className="storage-meter-rows">
        {counts.map(([label, n]) => (
          <li key={label}>
            <span>{label}</span>
            <b>{n} 条</b>
