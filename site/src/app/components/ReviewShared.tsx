/**
 * Shared shell primitives for the review/study pages (Results / Analytics /
 * History / Mistakes / Vocab). Ported from the desktop `components/Ui.tsx`
 * and `components/Shell.tsx`, minus the Tauri titlebar pieces. They live in
 * a Review* file because the components/ namespace is carved up per agent.
 */
import type { ReactNode } from "react";
import type { ModuleKind, SessionSummary } from "../types";
import { completion, isFinished } from "../lib/format";
...
const paths: Record<IconName, ReactNode> = {
  ...
  settings: <><circle cx="12" cy="12" r="3"/><path d="M19 13.5v-3l-2-.7a7 7 0 0 0-.8-1.8l.9-1.9-2.2-2.2-1.9.9a7 7 0 0 0-1.8-.8L10.5 2h-3l-.7 2a7 7 0 0 0-1.8.8l-1.9-.9L.9 6.1 1.8 8a7 7 0 0 0-.8 1.8l-2 .7v3l2 .7a7 7 0 0 0 .8 1.8l-.9 1.9 2.2 2.2 1.9-.9-.9-1.9a7 7 0 0 0-.8-1.8l2-.7Z" transform="translate(2.5) scale(.8)"/></>,
  ...
  contrast: <><circle cx="12" cy="12" r="8.5"/><circle cx="12" cy="12" r="8.5" fill="currentColor" stroke="none" opacity=".35"/></>,
...
export function ModuleIcon({ module, size = 44, className = "" }: { module: ModuleKind; size?: number; className?: string }) {
...
/**
 * How far through a paper a session got — the web port of the desktop
 * `ProgressBar` in components/ExamRows.tsx. Renders nothing when the record
 * predates answered/total tracking rather than inventing a bar.
 */
export function SessionProgress({ session }: { session: SessionSummary }) {
