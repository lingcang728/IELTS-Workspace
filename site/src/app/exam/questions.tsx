...
import { assetSrc } from "../content";
import { selectedLetters, toggleSharedLetter } from "../lib/choice";
import type { ExamMode, ExamSection, Question, QuestionGroup } from "../types";
...
function QuestionLayout({
  ...
  return (
    <div className="q-layout" aria-label="题目版式">
...
function QuestionInlineInput({
  ...
  if (type === "true_false_ng" || type === "yes_no_ng" || type === "single_choice" || type === "matching" || type === "labelling") {
    ...
    return (
      ...
      >
        <option value="">第{question.number}题</option>
        {options.map((option) => (
  ...
  return (
    <input
      ...
      placeholder={`第${question.number}题`}
    />
  );
}
...
function SharedMultiSelect({
  ...
  const rangeLabel =
    ordered.length === 1
      ? `第 ${ordered[0].number} 题`
      : `第 ${ordered[0].number} 至 ${ordered[ordered.length - 1].number} 题`;
  ...
  return (
    <div className={`q-block ${practice ? "q-card-block" : ""}`}>
      ...
      {practice ? <p className="choice-limit">已选 {picked.length} / {limit} 项</p> : null}
      {practice ? (
    ...
    if (practice) {
      return (
        <div className="q-block q-card-block" data-qid={question.id}>
          ...
          <p className="choice-limit">已选 {picked.length} 项</p>
            ...
            {options.map((opt) => {
              ...
              return (
                <button
                  ...
                  aria-pressed={on}
                  title={blocked ? "该选项已用于其他题目" : undefined}
      ...
      <div className="q-block" data-qid={question.id}>
        ...
        <div className="choices multi" role="group">
          {options.map((opt) => {
            ...
            const blocked = taken.has(opt.id) && !on;
            return (
              <label key={opt.id} className={`${on ? "on" : ""} ${blocked ? "taken" : ""}`} title={blocked ? "该选项已用于其他题目" : undefined}>
...
function MatchingBoard({
  ...
  return (
    <section className="q-group q-group-practice matching-board">
      ...
      <p className="match-hint">把标签拖到题目上，或先点标签再点答题位。键盘用户：每题下方也有下拉框可用。</p>
          ...
          >
            ...
            <div className="drop-slot">
              {picked ? (
                ...
                <span className="drop-ph">{armed ? "点击此处放入" : "拖一个标签到这里"}</span>
              )}
            </div>
